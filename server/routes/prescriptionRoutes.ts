...
export default function prescriptionRoutes(db: Database) {
    ...
    router.get('/user/:userId', async (req, res) => {
        try {
            ...
            const groupedPrescriptions = prescriptions.reduce((acc: any[], curr) => {
        ...
        try {
            ...
            const result = await db.run(
                `INSERT INTO prescriptions (user_id, date, remarks)
                 VALUES (?, ?, ?)`,
                [prescription.user_id, prescription.date, prescription.remarks]
            );
        ...
        try {
            ...
            await db.run(
                `UPDATE prescriptions 
                 SET date = ?, remarks = ?
                 WHERE id = ?`,
                [prescription.date, prescription.remarks, req.params.id]
            );
